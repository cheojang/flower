...
### ✨ 사이트 편집 모드 (`/admin/editor`) — 추천
실제 홈페이지 화면 그대로 보면서 **직접 눌러서** 수정합니다.
- **사진 변경**: 상품 사진 클릭 → 이미지 주소(URL) 붙여넣기 **또는** 내 사진 업로드
- **가격·이름·뱃지 변경**: 글자를 클릭해서 바로 수정 (자동 저장)
- **★ 추천**: 클릭 한 번으로 랜딩 추천 노출 on/off
- **상품 추가/삭제**: ＋ 카드 / 카드의 － 버튼
- **메뉴(카테고리) 추가/삭제**: 하단 ＋ 버튼 / 메뉴 옆 － 버튼

> 표(리스트) 형태로 관리하고 싶다면 `상품 목록`, `카테고리` 탭도 그대로 제공됩니다.
> 업로드한 사진은 `public/uploads`에 저장됩니다. (Vercel 등 서버리스 배포에서는
> 파일이 유지되지 않으니 이미지 주소(URL) 방식을 권장)
